...
import { loadArchive, loadFeed } from "../lib/feed";
...
// included — most recently shown first. These accumulate forever.
storiesRoutes.get("/archive", async (c) => {
  const db = getDb(c.env);
  return c.json({ stories: await loadArchive(db, c.get("userEmail")) });
});
