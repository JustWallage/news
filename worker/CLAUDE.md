...
| `EMAIL` (`send_email`), `EMAIL_FROM`            | binding + var | Cloudflare Email Sending for the email sign-in codes. Like `ai`, declared top-level + `production` and OMITTED in `e2e` (the fake email seam covers local/e2e). `makeEmailSender` (`lib/email.ts`) returns the fake ONLY in local/e2e and the real binding otherwise (`null` → route 503 when `EMAIL`/`EMAIL_FROM` absent). `EMAIL_FROM` must be on a domain onboarded to Email Sending (SPF/DKIM/DMARC) |
...
- Email sign-in (alongside Google, same email-only identity): `POST
/auth/email/{request,verify}`. `request` Turnstile-gates, mints a 6-digit code
  via `lib/email-login.ts` (stored ONLY as `sha256Hex(`${email}:${code}`)`,
  10-min TTL, 5-attempt cap, 60s resend cooldown, email normalized lower/trim),
  and emails it via the `lib/email.ts` seam. `verify` checks the code and mints
  the same session cookie as the Google callback. INVARIANT: `request` returns the
  code as `devCode` ONLY in local/e2e (the email fake delivers nothing); it is
  NEVER present in production. Expired codes are reaped by `purgeExpiredCodes` in
  the nightly maintenance purge.
...
- `index.ts` `scheduled` has ONE cron, `*/5 * * * *` → `runTelegramDigests` →
  ...
  and email sign-in codes, and clear expired Telegram link codes (telegram rows
  survive; only the stale code fields are cleared).
